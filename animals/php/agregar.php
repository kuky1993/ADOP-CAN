...
if(!empty($_POST)){
	if(isset($_POST["id_animal"]) &&isset($_POST["nombre"]) &&isset($_POST["peso"]) &&isset($_POST["edad"]) &&isset($_POST["descripcion"])){
		if($_POST["id_animal"]!=""&& $_POST["nombre"]!=""&&$_POST["peso"]!=""&&$_POST["edad"]!=""&&$_POST["descripcion"]!=""){
			...
			$sql = "insert into Animal(id_animal,nombre,peso,edad,descripcion,created_at) value (\"$_POST[id_animal]\",\"$_POST[nombre]\",\"$_POST[peso]\",\"$_POST[edad]\",\"$_POST[descripcion]\",NOW())";
			$query = $con->query($sql);
			if($query!=null){
